...
#filtro2: lista(any) (any any->bool) any -> lista(any)
...
#Nota: esta es la otra vesión de la abstracción funcional filtro (bloque de codigpo 15 de clase 11)
def filtro2(operador,unaLista,n): 
    if vacia(unaLista):
        return listaVacia 
    else:
        if operador(cabeza(unaLista),n):
            return crearLista(cabeza(unaLista),filtro2(operador,cola(unaLista), n))
        else:
            return filtro2(operador , cola(unaLista), n)
